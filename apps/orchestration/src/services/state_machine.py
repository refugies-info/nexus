"""Workflow state machine enforcing pipeline stage sequence."""
...
class WorkflowStateMachine:
    """State machine enforcing pipeline stage sequence."""

    def __init__(self):
        """Initialize state machine."""
        self.stage_sequence = STAGE_SEQUENCE
        self.stage_to_index = {stage: idx for idx, stage in enumerate(STAGE_SEQUENCE)}

    def validate_stage_transition(self, current_stage: str, next_stage: str) -> bool:
        ...
        """
        try:
            current_idx = self.stage_to_index.get(current_stage)
            next_idx = self.stage_to_index.get(next_stage)
            ...
            if next_idx != current_idx + 1:
                ...
                raise PipelineError(
                    ...
                    f"Next stage must be {self.stage_sequence[current_idx + 1].value}"
                )
    ...
    def get_next_stage(self, current_stage: str) -> str:
        ...
        try:
            ...
            next_stage = self.stage_sequence[current_idx + 1].value
            logger.debug(
    ...
    def get_stage_index(self, stage: str) -> int:
        ...
        """
        try:
            idx = self.stage_to_index.get(stage)
    ...
    def get_remaining_stages(self, current_stage: str) -> list[str]:
        ...
        try:
            ...
            remaining = [stage.value for stage in self.stage_sequence[current_idx + 1 :]]

            logger.debug(
                "Remaining stages determined",
                extra={"current_stage": current_stage, "remaining_count": len(remaining)},
            )
            return remaining
        except PipelineError:
            raise
    ...
    def get_all_stages(self) -> list[str]:
        ...
        """
        return [stage.value for stage in self.stage_sequence]
    ...
    def get_stage_count(self) -> int:
        ...
        """
        return len(self.stage_sequence)

    def validate_workflow_state(
        self, current_stage: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        """
        try:
            if not self.is_valid_stage(current_stage):
                logger.error(
            ...
            current_idx = self.get_stage_index(current_stage)
            is_final = self.is_final_stage(current_stage)
            remaining = self.get_remaining_stages(current_stage) if not is_final else []

            state_info = {
                "current_stage": current_stage,
                "current_index": current_idx,
                "total_stages": self.get_stage_count(),
                "progress_percentage": int((current_idx + 1) / self.get_stage_count() * 100),
