...
async def validate_program(
    ...
    Args:
        ...
        policy_rules: List of policy rules to check
    ...
    try:
        ...
        for rule in policy_rules:
            is_violated = _check_rule(program_data, rule)
...
async def check_applicable_rules(
    program_data: ProgramData,
    policy_rules: PolicyRules,
) -> PolicyRules:
    ...
    Args:
        ...
        policy_rules: All available policy rules
    ...
    try:
        ...
        applicable = [rule for rule in policy_rules if _is_rule_applicable(program_data, rule)]

        logger.debug(
            "Policy rules checked",
            extra={"applicable_count": len(applicable)},
        )
...
async def reject_program(
    ...
    Args:
        ...
        repo: Policy repository for storing decision
    ...
    try:
        ...
        await repo.create_policy_decision(
            program_id=program_id,
            decision="rejected",
            reason=reason,
            violations=violations,
...
def _check_rule(program_data: ProgramData, rule: PolicyRule) -> bool:
    ...
    try:
        ...
        checkers: dict[str, Callable] = {
            "for_profit": _check_for_profit_rule,
            "temporary": _check_temporary_rule,
            "out_of_scope_public": _check_out_of_scope_public_rule,
            "specialized_structure": _check_specialized_structure_rule,
        }

        checker = checkers.get(rule_category)
        if not checker:
            logger.debug(
                "Unknown rule category",
                extra={"category": rule_category},
            )
            return False

        return checker(program_data, rule_criteria)

    except Exception as e:
        logger.error(
            "Failed to check policy rule",
            extra={"error": str(e), "rule_id": rule.get("id")},
        )
        return False
...
def _check_out_of_scope_public_rule(
    program_data: ProgramData,
    criteria: dict[str, Any],
) -> bool:
    ...
    return any(public in out_of_scope_publics for public in target_publics)


def _check_specialized_structure_rule(
    program_data: ProgramData,
    criteria: dict[str, Any],
) -> bool:
