...
4. **Given** the monorepo structure, **When** I run `uv run python -m libs.pipeline`, **Then** Python can resolve imports from the `libs/` directory
...
3. **Given** the directory structure, **When** I navigate to `libs/pipeline/src/`, **Then** I see subdirectories for each pipeline stage (ingestion, reconciliation, enrichment, langage_clair, translation, validation, publication)
...
4. **Given** the testing infrastructure, **When** I create a test file in `libs/pipeline/tests/`, **Then** pytest discovers and runs it
...
- **FR-005**: `libs/pipeline/` MUST contain subdirectories for each pipeline stage: `ingestion/`, `reconciliation/`, `enrichment/`, `langage_clair/`, `translation/`, `validation/`, `publication/`
- **FR-006**: Each pipeline stage directory MUST have a `tests/` subdirectory for stage-specific tests

#### Python Configuration
- **FR-007**: Repository MUST have a `pyproject.toml` file configuring the Python workspace
- **FR-008**: `pyproject.toml` MUST specify Python 3.12+ as the minimum version
- **FR-009**: `pyproject.toml` MUST configure uv as the package manager
- **FR-010**: `pyproject.toml` MUST configure ruff for linting and formatting
- **FR-011**: `pyproject.toml` MUST configure pytest as the testing framework
- **FR-012**: `pyproject.toml` MUST configure mypy for type checking
- **FR-013**: Python packages MUST be installable via `uv sync`

#### Node.js Configuration
- **FR-014**: Repository MUST have a `package.json` file at the root
- **FR-015**: Repository MUST have a `pnpm-workspace.yaml` file defining the monorepo workspaces
- **FR-016**: `package.json` MUST specify Node.js 22+ as the minimum version
- **FR-017**: Node.js packages MUST use pnpm as the package manager
- **FR-018**: Node.js tooling MUST use biome for linting and formatting

#### Code Quality Tools
- **FR-019**: Repository MUST have a `.pre-commit-config.yaml` file
- **FR-020**: Pre-commit hooks MUST include ruff for Python linting and formatting
- **FR-021**: Pre-commit hooks MUST include nbstripout for Jupyter notebook output removal
- **FR-022**: Pre-commit hooks MUST include trailing whitespace removal
- **FR-023**: Pre-commit hooks MUST include end-of-file fixer
- **FR-024**: Developers MUST be able to install pre-commit hooks via `uv run pre-commit install`

#### Git Configuration
- **FR-025**: Repository MUST have a `.gitignore` file excluding build artifacts, virtual environments, and notebook outputs
- **FR-026**: `.gitignore` MUST exclude Python cache directories (`__pycache__/`, `*.pyc`, `.pytest_cache/`)
- **FR-027**: `.gitignore` MUST exclude Node.js directories (`node_modules/`, `.pnpm-store/`)
- **FR-028**: `.gitignore` MUST exclude Jupyter notebook checkpoints (`.ipynb_checkpoints/`)
- **FR-029**: `.gitignore` MUST exclude environment files (`.env`, `.env.local`)

#### Task Runner
- **FR-030**: Repository MUST have a `justfile` providing common development commands
- **FR-031**: `justfile` MUST include commands for installing dependencies (e.g., `just install`)
- **FR-032**: `justfile` MUST include commands for running linters and formatters (e.g., `just lint`, `just format`)
- **FR-033**: `justfile` MUST include commands for running tests (e.g., `just test`)
- **FR-034**: `justfile` MUST include commands for setting up pre-commit hooks (e.g., `just setup`)
- **FR-035**: `justfile` MUST include a default recipe that lists all available commands
- **FR-036**: `justfile` MUST support both Python and Node.js tooling commands

#### Documentation
- **FR-037**: Repository MUST have a `CONTRIBUTING.md` file with development setup instructions
- **FR-038**: `CONTRIBUTING.md` MUST document how to install just and use the justfile
- **FR-039**: `CONTRIBUTING.md` MUST document how to install dependencies (uv and pnpm)
- **FR-040**: `CONTRIBUTING.md` MUST document available just commands
- **FR-041**: `CONTRIBUTING.md` MUST document the monorepo structure and conventions
