...
**Feature Branch**: `003-pipeline-orchestration`
**Created**: 2025-10-20
**Status**: Draft
...
As the Nexus system, I need to process new French learning programs from Data Inclusion through all pipeline stages (ingestion → reconciliation → enrichment → langage clair → translation → validation → publication) so that accurate, multilingual information sheets are published to Réfugiés.info.

**Why this priority**: Foundation for all pipeline functionality - without this, no information sheets can be generated. This is the core value proposition of Nexus.

**Independent Test**: Can be fully tested by submitting a new program and verifying it progresses through all 7 stages, with state tracked at each stage, and delivers a published information sheet.

**Acceptance Scenarios**:

1. **Given** a new French learning program from Data Inclusion, **When** the pipeline processes it, **Then** the program progresses through all 7 stages sequentially and state is tracked in the database
2. **Given** a program at the enrichment stage, **When** a stage fails with a transient error, **Then** the system retries the stage automatically without manual intervention
3. **Given** a program at the validation stage, **When** quality checks fail, **Then** the program is routed to the editorial review queue and does not proceed to publication
4. **Given** multiple programs being processed, **When** one program fails, **Then** other programs continue processing without interruption
...
- Pipeline stages (ingestion, reconciliation, enrichment, langage clair, translation, validation, publication) will be implemented as separate features following this orchestration infrastructure
