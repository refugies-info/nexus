...
- **Information Sheet**: Represents a program's information sheet with progressive enhancement. Tracks program ID (Data Inclusion unique ID as primary key), structure_id and id (composite key for Carif-Oref matching), current stage, status (draft/in_review/approved/published), and data added at each stage (ingested, validated_policy, reconciled, enriched, langage_clair, translated, validated, published).
